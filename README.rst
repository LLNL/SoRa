...
usage: doSR.py [options] configFile
...
test_sr/HARMTFconfig.json is the file I used for most of my symbolic
regression testing.  It works quite well and keeps bloat to a minimum.
...
HARMTFconfig.json has.  So let's try it.  Our MPI cluster has 20
...
symbolic regression as they concentrate very strongly and the points
...
next island.  Generally good migrants should be selected.
  ...
  3j.3: replacementSelect determines which individuals will be
...
the size of the population, how may generations, how much crossover and
...
paper explains the details.
Gardner, M. A., Gagné, C., & Parizeau, M. (2015). Controlling code
...
fullRandom removes points from the data set complete randomly.  (For
...
column in the data is the X axis, and which is the Yaxis.  Then it
